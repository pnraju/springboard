...
SELECT COUNT(*) -- 4 facilities do not charge a fee to members
FROM Facilities
WHERE membercost = 0.0;
...
the member name. */
SELECT DISTINCT name AS court, CONCAT(firstname, " ", surname) AS member
FROM Bookings
...
SELECT name AS facility, SUM(slots * 0.5) AS guest_usage_hours
FROM Facilities
INNER JOIN Bookings
    USING (facid)
WHERE memid = 0
